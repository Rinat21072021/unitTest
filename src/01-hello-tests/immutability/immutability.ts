...
export const replaceBooks = (user: UserWhithLaptopType & UserWhithBooksType, oldBook: string, NewBook: string) => {
    return {...user, books: user.books.map(m => m === oldBook ? NewBook : oldBook)}
}
export const removeBooks = (user: UserWhithLaptopType & UserWhithBooksType, removeBook: string) => {
    return {...user, books: user.books.filter(f => f !== removeBook)}
}
